...
- Name: 02-master-data.spec.js >> TS_MASTER_DATA: Master Data Management >> TC_MD_03: Create and Delete Subject
- Location: tests/e2e/02-master-data.spec.js:81:3
...
Received string:  "new row violates row-level security policy for table \"subjects\""
Timeout: 5000ms

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('[role="status"]').first()
    9 × locator resolved to <div role="status" aria-live="polite" class="go3958317564">new row violates row-level security policy for ta…</div>
      - unexpected value "new row violates row-level security policy for table "subjects""

```

```yaml
- status: new row violates row-level security policy for table "subjects"
```
