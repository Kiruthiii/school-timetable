...
- Name: 02-master-data.spec.js >> TS_MASTER_DATA: Master Data Management >> TC_MD_01: Create and Delete Teacher
- Location: tests/e2e/02-master-data.spec.js:10:3
...
Received string:  "new row violates row-level security policy for table \"teachers\""
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[role="status"]').first()
    8 × locator resolved to <div role="status" aria-live="polite" class="go3958317564">new row violates row-level security policy for ta…</div>
      - unexpected value "new row violates row-level security policy for table "teachers""

```

```yaml
- status: new row violates row-level security policy for table "teachers"
```
